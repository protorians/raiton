...
function logValidationError(options: CliOptions, invalid: CommitInfo[]) {
    ...
    Logger.error("Erreur : certains commits ne respectent pas le format conventional commits.");
    Logger.error('Format attendu : "type(scope)!: description" (ex: "feat(auth): add login")');
    Logger.error("");
    for (const commit of invalid) {
        Logger.error(`  - ${commit.subject}`);
    }
    Logger.error("");
    Logger.error(
...
function log(options: CliOptions, data: Record<string, unknown>) {
    ...
    if (data.status === "skipped") {
        Logger.log(`Aucune mise à jour : ${data.reason}`);
        return;
    }
    Logger.log(`Tag le plus récent : ${data.latestTag ?? "aucun"}`);
    Logger.log(`Commits analysés : ${data.commitCount}`);
    Logger.log(`Increment déterminé : ${data.increment}`);
